...
# 2026-06-24 续：定位真 bug（flow-SDE 第一步 sigma 爆炸）+ 4 个 RL 实验

所有 eval 同协议：**240 步 / fix 代码 / RLinf eval harness / mujoco 3.3.2**。
RL 统一参数：noise 0.3 / lr 1e-5 / clip_grad 100 / GRPO group_size 8 / 从 released 起。

## 核心 bug：flow-SDE 第一步 sigma 爆炸

`fastwam_rl.py` 的 flow-SDE 在**第一个去噪步**（t≈1，t_model=num_train_timesteps）
`sigma = noise_level × √(t/(1-t))` 爆炸：`_T_EPS=1e-4` 把 1−t clamp 到 1e-4，
sigma 冲到 noise×100 → 第一步 action 噪声 std≈2.2（action 范围仅 [-1,1]），动作被随机化、
rollout 全失败。dexbotic/gr00t 用 timesteps[1] 兜底所以稳，FastWAM 没有。

- **training**：noise 0.15 训练 SR 崩到 0.016、0.2 直接 NaN（对 noise 异常敏感）。
- **eval**：deterministic eval 的 mean 含 sigma correction（eval noise=0.1）→ 旧 eval 被系统性低估。
- 被 `filter_rewards` NaN-guard 静默吞成 no-op，延误定位。

**修复**：`_SIGMA_T_MAX = 0.98`，第一步 t clamp 到 0.98 → std 2.21→0.155。
验证：noise 0.15 训练 SR 0.016→0.93；noise 0.3 NaN→健康 SR 0.83。

## ① spatial task4 RL（同协议 240步/100ep）
| | SR |
|---|---|
| released @240 | 0.89 |
| **task4-RL(v4) @240** | **0.97** |

净 **+8pt**。（旧 bug @400步=0.87 < fix @240步=0.89，确认 fix 让 eval 回升。）

## ② 单任务 RL 泛化损失（cross-task 全 10 task × 50 trials）
| task | released | task4-RL | Δ |
|---|---|---|---|
| **4 ★** | 0.86 | **1.00** | **+0.14** |
| **8** | 0.96 | 0.80 | **−0.16** |
| 0/1/2/6/7 | 0.98-1.0 | 0.90-0.98 | −0.02~−0.08 |
| 总体 | 0.97 | 0.954 | −0.016 |

target 大涨 +14、task8 退化 −16，总体几乎持平 → 单任务 RL 典型泛化代价。

## ③ libero_90 eval（released，10×90=900ep）
released（**libero_spatial 专训**）在 libero_90 整体 **0.079** —— 90 task 大多没见过、几乎全失败，spatial 专训 ckpt 不通用 libero_90。

## ④ libero_90 task28 RL（跨 suite headroom task）
候选全量验证：task28=0.48、task22=0.35 → 选 task28。
| | SR |
|---|---|
| task28 base | 0.48 |
| **task28-RL step50（确定性 eval）** | **0.82** |

净 **+34pt**（确定性 0.82 ≈ 训练峰值 0.81）。验证 RL pipeline 跨 suite 普适。

## 保留 ckpt（FastWAM 格式）
- `results/rl_h240_ep4/step50_fastwam.pt`、`results/rl_task4_v4/step50_fastwam.pt`、`results/rl_lib90_t28/step50_fastwam.pt`
- `fastwam_ckpt/libero_uncond_2cam224.pt`(released)、`robotwin_uncond_3cam_384.pt`

## 代码改动（待 commit）
- **RLinf**：`fastwam_rl.py` sigma fix（`_SIGMA_T_MAX`）+ per-task eval breakdown + filter_rewards NaN guard + ratio fix（in-process recompute prev_logprob）
- **install**：pin `mujoco==3.3.2`（LIBERO 资产录制版本，3.9 掉 5-8pt）
